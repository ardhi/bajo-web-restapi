...
async function routeByVerb ({ file, ctx, appCtx, dir, pathPrefix, ns, alias }) {
  const { importModule, parseObject, isSet } = this.app.bajo
  const { methodMap, getParams } = this.app.bajoWeb
  ...
  mod.url = mod.url ?? url
  mod.method = method
  const swagger = this.app.bajoWebRestapiSwagger
  const defSchema = {
    description: swagger ? swagger.docSchemaDescription(action) : undefined,
  ...
  mod.handler = async function (req, reply) {
    ...
    return this.transformResult({ data, req, reply, options })
  }
  mod.config = mod.config ?? {}
  mod.config.pathSrc = url
  return serveRoute.call(me, { mod, ns })
}
