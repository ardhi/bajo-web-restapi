...
async function routeByRepoBuilder ({ file, ctx, childCtx, dir, pathPrefix, plugin, prefix, appPrefix }) {
  ...
  const { methodMap, docSchema } = this.bajoWebRestapi.helper
  ...
  for (const method of methods) {
    ...
    customMod.schema = await docSchema({ repo: builder.repo, method, ctx })
    merge(mod, customMod, {
