async function xml (req, reply, payload) {
  ...
  const { XMLBuilder } = await importPkg('bajoExtra:fast-xml-parser', { returnDefault: false })
  const { get, set, isPlainObject, isArray } = await importPkg('lodash-es')
  const cfg = getConfig('bajoWebRestapi')
  const { wrapper, valueAsAttributes, declaration } = cfg.format.xml.response
  ...
  const dataKey = cfg.responseKey.data
