...
async function routeByCollBuilder ({ file, ctx, dir, pathPrefix, ns }) {
  ...
  for (const method of methods) {
    ...
    customMod.schema = customMod.schema ?? await this.docSchemaColl({ coll: builder.coll, method, ctx, options: { hidden: builder.hidden } })
    ...
    merge(mod, customMod, {
      ...
      handler: async function (req, reply) {
        const helper = me.app.bajoWeb[camelCase(`record ${method === 'replace' ? 'update' : method}`)]
