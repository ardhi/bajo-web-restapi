...
async function routeByCollBuilder ({ file, ctx, childCtx, dir, pathPrefix, plugin }) {
  ...
  const { importPkg, readConfig, getConfig, parseObject } = this.bajo.helper
  ...
  for (const method of methods) {
    ...
    merge(mod, customMod, {
      ...
      handler: async function (req, reply) {
        ...
        if (headers['x-data-only']) dataOnly = headers['x-data-only']
        if (headers['x-no-count']) noCount = headers['x-no-count']
