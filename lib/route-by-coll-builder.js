...
async function routeByCollBuilder ({ file, ctx, childCtx, dir, pathPrefix, plugin }) {
  ...
  const { importPkg, readConfig, getConfig, parseObject, isSet } = this.bajo.helper
  ...
  for (const method of methods) {
    ...
    customMod.schema = customMod.schema ?? await docSchemaColl({ coll: builder.coll, method, ctx })
    ...
    merge(mod, customMod, {
      ...
      handler: async function (req, reply) {
        ...
        let { fields, dataOnly, noCount } = await getParams(req)
        const headers = parseObject(req.headers, true, true)
        if (isSet(headers['x-data-only'])) dataOnly = headers['x-data-only']
        if (isSet(headers['x-no-count'])) noCount = headers['x-no-count']
        const options = { fields, dataOnly, noCount }
